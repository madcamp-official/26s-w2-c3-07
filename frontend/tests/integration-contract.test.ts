...
describe('frontend API integration contracts', () => {
  ...
  it('shows server-returned clue summaries and refreshes the clue cache once', () => { const source = read('app/game/[sessionId]/interrogation/[suspectId]/page.tsx'); expect(source).toContain('result.newlyUnlockedClues'); expect(source).toContain('<ClueUnlockNotice'); expect(source.match(/clues\.reload\(\)/g)?.length).toBe(1); });
