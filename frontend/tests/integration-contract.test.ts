...
describe('frontend API integration contracts', () => {
  ...
  it('renders Korean interrogation labels while retaining English state codes', () => { const source = read('app/game/[sessionId]/interrogation/[suspectId]/page.tsx'); expect(source).toContain('message.emotionAfterLabel || message.emotionAfter'); expect(source).toContain('message.questionTypeLabel || message.questionType'); });
