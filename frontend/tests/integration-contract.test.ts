...
describe('frontend API integration contracts', () => {
  ...
  it('submits only selected session evidence and clears it after success', () => { const source = read('app/game/[sessionId]/interrogation/[suspectId]/page.tsx'); expect(source).toContain('`/sessions/${sessionId}/evidence`'); expect(source).toContain('presentedEvidenceIds: selectedEvidenceIds'); expect(source.indexOf('await api.post<InterrogationResponse>')).toBeLessThan(source.indexOf('setSelectedEvidenceIds([])')); });
  it('shows server-returned clue and evidence unlocks and refreshes each cache once', () => { const source = read('app/game/[sessionId]/interrogation/[suspectId]/page.tsx'); expect(source).toContain('result.newlyUnlockedClues'); expect(source).toContain('result.newlyUnlockedEvidence'); expect(source).toContain('<ClueUnlockNotice clues={unlockedClues} evidence={unlockedEvidence}'); expect(source.match(/clues\.reload\(\)/g)?.length).toBe(1); expect(source.match(/evidence\.reload\(\)/g)?.length).toBe(1); });
  it('distinguishes acquired evidence from viewed evidence on the session screen', () => { const source = read('app/game/[sessionId]/page.tsx'); expect(source).toContain("item.viewedAt ? '열람 완료' : '획득 · 미열람'"); expect(source).toContain('result.newlyUnlockedEvidence.length'); });
