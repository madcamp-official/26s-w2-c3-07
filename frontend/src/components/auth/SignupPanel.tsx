...
export function SignupPanel() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (password !== String(values.get('passwordConfirm'))) { setError('비밀번호가 일치하지 않습니다.'); return; }
    setSubmitting(true); setError('');
    try {
      await signUp(String(values.get('email')), password, String(values.get('displayName')));
      router.replace('/regions');
    } catch (cause) { setError(cause instanceof ApiError ? cause.message : '회원가입에 실패했습니다.'); }
  ...
  return <div className="relative w-full max-w-md border border-brass-600/50 bg-noir-800/90 p-8 shadow-2xl">
    ...
    <p className="mt-6 text-center text-sm text-parchment-300/70">이미 등록됐나요? <Link href="/login" className="text-brass-400 hover:underline">신원 확인하기</Link></p>
  </div>;
}
