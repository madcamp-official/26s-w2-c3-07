...
export default function EpisodePage() {
  const { episodeId: episodeKey } = useParams<{ episodeId: string }>();
  const router = useRouter();
  const detail = useApiResource<EpisodeDetail>(`/episodes/${episodeKey}`);
  const scene = useApiResource<Scene>(`/episodes/${episodeKey}/scene`);
  const suspects = useApiResource<PublicSuspect[]>(`/episodes/${episodeKey}/suspects`);
  const [difficulty, setDifficulty] = useState<'easy' | 'normal' | 'hard'>('normal');
  const [starting, setStarting] = useState(false); const [startError, setStartError] = useState<ApiError | null>(null);
  async function start() { if (starting || !detail.data) return; setStarting(true); setStartError(null); try { const session = await api.post<SessionView>('/sessions', { episodeId: detail.data.id, difficulty }); router.push(`/game/${session.sessionId}`); } catch (cause) { setStartError(cause as ApiError); setStarting(false); } }
  return <AuthGuard><main className="min-h-screen bg-noir-950 px-6 py-10 text-parchment-100"><div className="mx-auto max-w-3xl space-y-7">
    ...
    {detail.loading ? <LoadingState /> : detail.error ? <ErrorState error={detail.error} retry={detail.reload} /> : detail.data ? <>
      ...
      {suspects.data && <section><h2 className="mb-3 font-display text-2xl">용의자 {suspects.data.length}명</h2><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{suspects.data.map((item) => <div key={item.id} className="overflow-hidden border border-brass-600/30 bg-noir-900/70"><SuspectImage imageUrl={item.imageUrl} name={item.name} sizes="(min-width: 768px) 25vw, 50vw" className="aspect-square w-full" /><p className="px-3 py-2 text-center font-display">{item.name}</p></div>)}</div><Link className="mt-3 inline-block text-sm text-brass-400" href={`/episodes/${episodeKey}/suspects`}>용의자 공개 정보 보기 →</Link></section>}
