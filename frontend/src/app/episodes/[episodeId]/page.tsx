...
export default function EpisodePage() {
  ...
  return <AuthGuard><main className="min-h-screen bg-noir-950 px-6 py-10 text-parchment-100"><div className="mx-auto max-w-3xl space-y-7">
    ...
    {detail.loading ? <LoadingState /> : detail.error ? <ErrorState error={detail.error} retry={detail.reload} /> : detail.data ? <>
      ...
      {suspects.data && <section><h2 className="mb-3 font-display text-2xl">용의자 {suspects.data.length}명</h2><div className="grid grid-cols-2 gap-4 md:grid-cols-3">{suspects.data.map((item) => <div key={item.id} className="overflow-hidden border border-brass-600/30 bg-noir-900/70"><SuspectImage imageUrl={item.imageUrl} name={item.name} sizes="(min-width: 768px) 33vw, 50vw" className="aspect-[4/5] w-full" /><p className="px-3 py-3 text-center font-display text-lg">{item.name}</p></div>)}</div><Link className="mt-3 inline-block text-sm text-brass-400" href={`/episodes/${episodeId}/suspects`}>용의자 공개 정보 보기 →</Link></section>}
