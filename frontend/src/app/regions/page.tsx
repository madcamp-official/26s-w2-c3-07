...
export default function RegionsPage() {
  ...
  const solvedCount = progress.data ? progress.data.solvedEpisodeCount : 0;
  const totalCount = progress.data ? progress.data.regionProgress.reduce((sum, item) => sum + item.totalEpisodes, 0) : 0;
  const dialectCount = progress.data ? progress.data.unlockedDialectCount : 0;
  ...
  return <AuthGuard><main className="min-h-screen bg-noir-950 px-6 py-10 text-parchment-100"><div className="mx-auto max-w-6xl space-y-8">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs tracking-widest text-evidence-red">CASE ARCHIVE</p><h1 className="font-display text-4xl font-bold">지역 및 사건 선택</h1></div><div className="flex items-start gap-4"><CaseArchiveCard solvedCount={solvedCount} totalCount={totalCount} cluesCollected={dialectCount} cluesTotal={dialectCount}/><Link href="/profile" className="border border-brass-600/50 px-4 py-2">내 기록</Link></div></header>
