...
export default function InterrogationPage() {
  ...
  async function send(event: FormEvent) { event.preventDefault(); if (disabled || question.trim().length < 2) return; const id = requestId ?? crypto.randomUUID(); setRequestId(id); setSending(true); setError(null); try { const result = await api.post<InterrogationResponse>(`/sessions/${sessionId}/interrogations`, { requestId: id, suspectId, question: question.trim(), presentedEvidenceIds: evidenceId ? [evidenceId] : [] }); const notices = [result.newlyUnlockedClues.length ? `새 단서 ${result.newlyUnlockedClues.length}개` : '', result.newlyUnlockedEvidence.length ? `새 증거 ${result.newlyUnlockedEvidence.length}개` : ''].filter(Boolean); setNotice(notices.length ? `${notices.join(', ')}를 획득했습니다.` : ''); setQuestion(''); setRequestId(null); await Promise.all([messages.reload(), session.reload(), clues.reload()]); } catch (cause) { setError(cause as ApiError); } finally { setSending(false); } }
