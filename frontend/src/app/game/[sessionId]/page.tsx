...
export default function GamePage() {
  ...
  return <AuthGuard><main className="min-h-screen bg-noir-950 px-6 py-10 text-parchment-100"><div className="mx-auto max-w-5xl space-y-7">
    {session.error ? <ErrorState error={session.error} retry={session.reload} /> : session.data && <>
      ...
      <section><h2 className="mb-3 font-display text-2xl">획득한 증거</h2>{!evidence.data?.length ? <EmptyState label="현재 획득한 증거가 없습니다." /> : <div className="grid gap-3 md:grid-cols-2">{evidence.data.map((item) => <button key={item.id} onClick={() => view(item)} disabled={Boolean(busy) || Boolean(terminalMessage[session.data!.status])} className="border border-brass-600/30 p-4 text-left disabled:opacity-50"><b>{item.title}</b><span className="float-right text-xs">{item.viewedAt ? '열람 완료' : '획득 · 미열람'}</span><p className="mt-2 text-sm opacity-60">{item.description}</p></button>)}</div>}</section>
