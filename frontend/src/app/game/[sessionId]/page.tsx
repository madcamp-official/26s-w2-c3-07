...
export default function GamePage() {
  ...
  async function choose(suspectId: string) { if (busy) return; setBusy(suspectId); setActionError(null); try { await api.patch(`/sessions/${sessionId}/current-suspect`, { suspectId }); router.push(`/game/${sessionId}/interrogation/${suspectId}`); } catch (e) { setActionError(e as ApiError); setBusy(''); } }
  async function view(item: Evidence) { if (busy) return; setBusy(item.id); try { const result = await api.post<EvidenceViewResult>(`/sessions/${sessionId}/evidence/${item.id}/view`); setNotice(result.newClueIds.length ? `새 단서 ${result.newClueIds.length}개를 획득했습니다.` : '증거를 확인했습니다.'); await Promise.all([evidence.reload(), clues.reload(), session.reload()]); } catch (e) { setActionError(e as ApiError); } finally { setBusy(''); } }
  ...
  return <AuthGuard><main className="min-h-screen bg-noir-950 px-6 py-10 text-parchment-100"><div className="mx-auto max-w-5xl space-y-7">
    {session.error ? <ErrorState error={session.error} retry={session.reload} /> : session.data && <>
      ...
      <section><h2 className="mb-3 font-display text-2xl">열람 가능한 증거</h2>{!evidence.data?.length ? <EmptyState label="현재 열람 가능한 증거가 없습니다." /> : <div className="grid gap-3 md:grid-cols-2">{evidence.data.map((item) => <button key={item.id} onClick={() => view(item)} disabled={Boolean(busy) || Boolean(terminalMessage[session.data!.status])} className="border border-brass-600/30 p-4 text-left disabled:opacity-50"><b>{item.title}</b><span className="float-right text-xs">{item.viewedAt ? '열람 완료' : '열람 가능'}</span><p className="mt-2 text-sm opacity-60">{item.description}</p></button>)}</div>}</section>
      <section><h2 className="mb-3 font-display text-2xl">심문할 용의자</h2>{suspects.loading ? <LoadingState /> : !suspects.data?.length ? <EmptyState label="용의자 정보가 없습니다." /> : <div className="grid gap-5">{suspects.data.map((suspect) => { const state = session.data!.suspectStates.find((item) => item.suspectId === suspect.id); return <button key={suspect.id} onClick={() => choose(suspect.id)} disabled={Boolean(busy) || Boolean(terminalMessage[session.data!.status]) || session.data!.remainingQuestions === 0} className="flex overflow-hidden border border-brass-600/30 bg-noir-900/70 text-left disabled:opacity-40"><SuspectImage imageUrl={suspect.imageUrl} name={suspect.name} sizes="240px" className="aspect-square w-60 shrink-0" /><div className="flex flex-1 flex-col justify-center p-6"><h3 className="font-display text-2xl">{suspect.name}</h3><p className="mt-1 text-base opacity-60">{suspect.occupation}</p><p className="mt-3 text-sm">감정 {state?.emotion ?? suspect.initialEmotion} · 질문 {state?.questionsAsked ?? 0}회</p></div></button>; })}</div>}</section>
