...
| 이름 | 학교 | GitHub | 역할 |
|---|---|---|---|
| 안소희 | - | `soheean1370` | 공동 기획, 사건·사투리 콘텐츠 구성, 화면 기획 및 프론트엔드 구현 |
| 손기환 | - | `Kihwan819` | 공동 기획, 프론트엔드·백엔드 구현, API·DB 통합 및 배포 안정화 |
...
> “형사님, 그 뜻이 아니예라”를 만들며 가장 인상 깊었던 점은 사투리가 단순히 대사에 지역색을 더하는 장식이 아니라 추리의 재료가 될 수 있다는 사실이었습니다. 용의자의 “됐다 고마”라는 짧은 말도 단순한 거절인지, 불편한 사실을 덮으려는 회피인지에 따라 전혀 다른 단서가 됩니다. 플레이어가 표현의 표면과 진의 사이를 스스로 해석하도록 사건과 대사를 구성하면서, 우리가 익숙하게 듣던 말투 안에도 충분한 미스터리와 서사가 담길 수 있다는 것을 체감했습니다. 짧은 기간 동안 기획과 콘텐츠 제작을 함께 밀어붙이는 과정은 쉽지 않았지만, 작은 표현 하나까지 반복해서 다듬는 일이 결국 게임의 몰입도와 완성도를 만든다는 점을 배웠습니다.

**손기환:**

> 프로젝트를 진행하며 AI 이미지 생성 프롬프트도 하나의 정교한 설계 문서라는 것을 배웠습니다. 처음에는 “증거 사진을 만들어 달라”는 정도로 요청했지만, 맥락이 부족하면 AI가 사건과 무관한 지역명을 만들거나 두 인물의 얼굴과 특징을 뒤섞었습니다. 이를 해결하기 위해 이미지 속 문구와 지명뿐 아니라 인물의 체형, 복장, 구도와 소품까지 구체적으로 언어화했습니다. 결국 AI에게 맡긴다는 것은 필요한 맥락을 사람이 얼마나 명확하게 정리해 전달하느냐의 문제였습니다. 기획부터 API, 데이터, 이미지와 게임 흐름을 짧은 시간 안에 연결하면서, 완성도는 큰 기능의 개수보다 서로 어긋나는 작은 디테일을 끝까지 찾아 고치는 데서 나온다는 점이 가장 크게 남았습니다.
