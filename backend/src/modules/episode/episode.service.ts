...
export const episodeService = {
  ...
  async detail(episodeKey: string) { const episode = await episodeRepository.findByKey(episodeKey); if (!episode) throw new AppError(404, 'Episode not found', 'EPISODE_NOT_FOUND'); const [victim, difficulties] = await Promise.all([episodeRepository.findVictim(episode.id), episodeRepository.findDifficulties(episode.id)]); if (!victim) throw new AppError(404, 'Episode not found', 'EPISODE_NOT_FOUND'); const { sceneDescription: _scene, progressStatus: _progress, ...publicEpisode } = episode; return { ...publicEpisode, victim, difficulties }; },
  async difficulties(episodeKey: string) { const episode = await episodeRepository.findByKey(episodeKey); if (!episode) throw new AppError(404, 'Episode not found', 'EPISODE_NOT_FOUND'); return episodeRepository.findDifficulties(episode.id); },
  async scene(episodeKey: string) { const episode = await episodeRepository.findByKey(episodeKey); if (!episode) throw new AppError(404, 'Episode not found', 'EPISODE_NOT_FOUND'); const [victim, parts] = await Promise.all([episodeRepository.findVictim(episode.id), episodeRepository.findSceneParts(episode.id)]); if (!victim) throw new AppError(404, 'Episode not found', 'EPISODE_NOT_FOUND'); return { narrative: episode.sceneDescription, victim, ...parts }; }
};
