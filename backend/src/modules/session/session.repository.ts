...
export const sessionRepository={
 ...
 async evidence(id:string){const {data,error}=await serviceRoleClient.from('session_evidence').select('evidence_id').eq('session_id',id).not('viewed_at','is',null);if(error)throw toAppError(error);return(data??[]).map(x=>x.evidence_id);},
