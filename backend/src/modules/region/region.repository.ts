...
type RegionRow = Pick<Row<'game_content', 'regions'>, 'id' | 'code' | 'name' | 'description' | 'image_url' | 'display_order' | 'is_active'>;
const columns = 'id, code, name, description, image_url, display_order, is_active';
const toRegion = (row: RegionRow): Region => ({
  id: row.id, code: row.code, name: row.name, description: row.description,
  imageUrl: row.image_url, displayOrder: row.display_order, isActive: row.is_active
});
const uuidPattern = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

export const regionRepository = {
  async findAll(): Promise<Region[]> {
    const { data, error } = await serviceRoleClient.schema('game_content').from('regions').select(columns).eq('is_active', true).order('display_order', { ascending: true });
