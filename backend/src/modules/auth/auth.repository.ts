...
const toSettings = (row: { user_id: string; sfx_enabled: boolean; bgm_enabled: boolean; locale: string }): UserSettings => ({
  userId: row.user_id,
...
export const authRepository = {
  ...
  async signUp(input: SignUpInput): Promise<SignUpResult> {
    ...
    try {
      const [{ data: profileRow, error: profileError }, { data: settingsRow, error: settingsError }] = await Promise.all([
        ...
        serviceRoleClient.from('user_settings').select('user_id, sfx_enabled, bgm_enabled, locale').eq('user_id', userId).single()
      ]);
  ...
  async getSettings(userId: string): Promise<UserSettings> {
    const { data, error } = await serviceRoleClient.from('user_settings').select('user_id, sfx_enabled, bgm_enabled, locale').eq('user_id', userId).maybeSingle();
  ...
  async updateSettings(userId: string, input: UpdateSettingsInput): Promise<UserSettings> {
    const changes = { ...(input.soundEnabled !== undefined && { sfx_enabled: input.soundEnabled }), ...(input.musicEnabled !== undefined && { bgm_enabled: input.musicEnabled }), ...(input.locale !== undefined && { locale: input.locale }), updated_at: new Date().toISOString() };
    const { data, error } = await serviceRoleClient.from('user_settings').update(changes).eq('user_id', userId).select('user_id, sfx_enabled, bgm_enabled, locale').maybeSingle();
