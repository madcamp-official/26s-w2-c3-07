...
const columns = 'id, episode_id, code, name, age, occupation, public_profile, personality, victim_relation, initial_emotion, display_order, image_url';
...
const map = (row: Record<string, unknown>): SuspectRecord => ({
  ...
  occupation: row.occupation as string | null, publicProfile: filterProfile(row.public_profile as Json), personality: filterProfile(row.personality as Json).summary ?? null,
  victimRelation: row.victim_relation as string | null, initialEmotion: String(row.initial_emotion), displayOrder: Number(row.display_order), imageUrl: row.image_url as string | null
});
