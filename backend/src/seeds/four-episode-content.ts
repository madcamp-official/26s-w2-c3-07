...
export const fourEpisodeContent = (): SeedTables => {
  ...
  specs.forEach((spec, episodeIndex) => {
    ...
    spec.clues.forEach(([title, description], index) => {
      ...
      tables.clue_unlock_conditions.push({ id: id(group, 500 + index), clue_id: clueIds[index], condition_type: 'evidence_discovered', condition_data: { evidence_id: evidenceIds[Math.min(index, evidenceIds.length - 1)] }, _target_evidence_id: evidenceIds[Math.min(index, evidenceIds.length - 1)], sort_order: 1 });
    });
