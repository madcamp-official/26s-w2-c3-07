...
export type Database = {
  public: {
    Tables: {
      ...
      game_sessions: Table<Id & { user_id: string; episode_id: string; difficulty_config_id: string; status: string; remaining_questions: number; started_at: string; expires_at: string | null; current_suspect_id: string | null; last_activity_at: string; completed_at: string | null; session_version: number } & Audit>;
      session_suspect_states: Table<Id & { session_id: string; suspect_id: string; current_emotion: string; current_emotion_ko: string | null; questions_used: number; interrogation_status: string; last_interrogated_at: string | null } & Audit>;
      interrogation_messages: Table<Id & { session_id: string; suspect_id: string; request_id: string; user_question: string; normalized_question: string | null; question_hash: string | null; question_type: string; question_type_ko: string | null; npc_response: string; emotion_before: string | null; emotion_before_ko: string | null; emotion_after: string | null; emotion_after_ko: string | null; evasion_type: string | null; used_fact_refs: string[]; revealed_fact_refs: string[]; claimed_fact_refs: string[]; presented_evidence_refs: string[]; question_cost: number; response_metadata: Json; created_at: string }>;
      session_evidence: Table<Id & { session_id: string; evidence_id: string; source_type: string; viewed_at: string | null }>;
  ...
  game_content: {
    Tables: {
      ...
      suspects: Table<Id & { episode_id: string; code: string; name: string; age: number | null; occupation: string | null; public_profile: Json; personality: Json; speech_style: Json; victim_relation: string | null; actual_route: Json; claimed_route: Json; initial_emotion: string; initial_emotion_ko: string | null; display_order: number; image_url: string | null; is_active: boolean } & Audit>;
      ...
      clue_unlock_conditions: Table<Id & { clue_id: string; group_no: number; condition_order: number; condition_type: ClueConditionType; target_ref: string | null; operator: 'EQ' | 'IN' | 'GTE' | 'EXISTS'; expected_value: Json; created_at: string }>;
      difficulty_initial_evidence: Table<Id & { difficulty_config_id: string; evidence_id: string; display_order: number; created_at: string }>;
      difficulty_initial_clues: Table<Id & { difficulty_config_id: string; clue_id: string; display_order: number; created_at: string }>;
      clue_evidence_unlocks: Table<Id & { clue_id: string; evidence_id: string; display_order: number; source_type: 'CLUE_UNLOCK'; created_at: string }>;
      ...
      suspect_response_rules: Table<Id & { suspect_id: string; question_type: string; question_type_ko: string | null; response_policy: Json; allowed_fact_refs: Json; hidden_fact_refs: Json; evasion_type: string | null; difficulty_overrides: Json } & Audit>;
      suspect_emotion_rules: Table<Id & { suspect_id: string; trigger_type: string; trigger_type_ko: string | null; from_emotion: string | null; from_emotion_ko: string | null; to_emotion: string; to_emotion_ko: string | null; condition: Json; priority: number } & Audit>;
  ...
  game_private: {
    Tables: {
      llm_request_logs: Table<Id & { session_id: string; message_id: string | null; purpose: string; model: string | null; status: string; latency_ms: number | null; prompt_tokens: number | null; completion_tokens: number | null; error_code: string | null; error_message: string | null; metadata: Json; question_type_ko: string | null; created_at: string }>;
    };
    Views: Record<never, never>; Functions: Record<never, never>; Enums: Record<never, never>; CompositeTypes: Record<never, never>;
  };
};
