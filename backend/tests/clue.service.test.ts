...
const clue = { id: clueId, code: 'GS-01-C1', title: '독성 물질', content: '차에서 독이 검출됐다.', description: '차에서 독이 검출됐다.', recordSummary: '찻잔에서 독 검출', clueType: 'CORE', importance: 'CORE', unlockedAt: new Date().toISOString(), source: 'EVIDENCE_VIEWED', sourceSuspect: null, sourceQuestion: null, sourceAnswer: null };
...
describe('clue SQL enforcement', () => {
  ...
  it('queries only columns present in the live session evidence schema', () => {
    ...
    expect(repositorySource).not.toContain('source_type, discovered_at');
  });
